...
const utils = require("./utils/lockup");
...
describe("Lockup and Registry", () => {
  ...
  it("Removes from the whitelist", async () => {
    ...
    for (const e of lockupAccount.whitelist) {
      await lockup.state.rpc.whitelistDelete(e, {
        accounts: {
          authority: provider.wallet.publicKey,
        },
      })
    ...
    lockupAccount = await lockup.state.fetch();
    assert.ok(lockupAccount.whitelist.length === 0);
  });
  ...
  it("Withdraws from the vesting account", async () => {
    ...
    it("Creates a member", async () => {
      ...
      assert.ok(memberAccount.metadata.equals(new anchor.web3.PublicKey(0)));
      assert.equal(

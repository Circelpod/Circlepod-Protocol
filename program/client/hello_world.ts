 ...
 * This file is created when running `solana program deploy target/deploy/bpf_program_template.so`
 */
...
export async function checkProgram(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to read program keypair at '${PROGRAM_KEYPAIR_PATH}' due to error: ${errMsg}. Program may need to be deployed with \`solana program deploy target/deploy/bpf_program_template.so\``,
    );
  }
  ...
  if (programInfo === null) {
    if (fs.existsSync(PROGRAM_SO_PATH)) {
      throw new Error(
        'Program needs to be deployed with `solana program deploy target/deploy/bpf_program_template.so`',
      );
    } else {
